...
async def send(message: str, image_directory: str = None, qq: str = None) -> None:
    ...
    if not cookies:
        ...
        while attempt < 3:
            try:
                ...
                print(f"登录尝试 {attempt + 1} 失败，进行重新登录后重试...")
                traceback.print_exc()
                attempt += 1
        else:
            print("连续 3 次登录失败，退出。")
    ...
    # 根据 submissions.json 生成图片文件列表
    image_files = []
    if os.path.exists(os.path.join(image_directory, "submissions.json")):
        try:
            with open(os.path.join(image_directory, "submissions.json"), 'r', encoding="utf-8") as f:
                submissions_data = json.load(f)
                # 获取 files 列表
                files = submissions_data.get("files", [])
                # 遍历每个文件并读取内容
            ...
            print("读取或解析 submissions.json 失败，使用默认文件列表...")
            # 如果无法读取 submissions.json，则列出所有图片文件
        ...
        # 如果没有 submissions.json，则列出所有图片文件
        for file_name in os.listdir(image_directory):
            if file_name.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
                file_path = os.path.join(image_directory, file_name)
                with open(file_path, 'rb') as img_file:
    ...
    while attempt < 3:
        try:
            tid = await qzone.publish_emotion(message, image_files)
